...
def compute_strain_tensor_3d(node_coords, node_displacements):
    """
    Computes the 3D Cauchy strain tensor (3x3) for an 8-node 
    ...
    # Evaluated at the center of the element (xi = 0, eta = 0, zeta = 0):
    # N_i = 1/8 * (1 + xi*xi_i) * (1 + eta*eta_i) * (1 + zeta*zeta_i)
    # dN_i / d_xi = 1/8 * xi_i * (1 + eta*eta_i) * (1 + zeta*zeta_i) -> for xi=eta=zeta=0 gives 1/8 * xi_i
    ...
    # 3x8 matrix of derivatives: [dN/d_xi; dN/d_eta; dN/d_zeta]
    ...
    # 3. Shape function derivatives in the global coordinate system [dN/dx, dN/dy, dN/dz]
...
def visualize_3d_deformation(nodes, u, epsilon):
    """
    Creates a 3D plot of the artificial skin hexahedron before and after deformation.
    """
    deformed_nodes = nodes + u
    
    # Edges connecting the 8 nodes into a hexahedron
    edges = [
    ...
    # Draw original edges (black dashed)
    for edge in edges:
        ax.plot3D(*zip(*nodes[edge]), color='black', linestyle='--', alpha=0.6, label='Original' if edge == [0,1] else "")
        
    # Draw deformed edges (red solid)
    for edge in edges:
    ...
    ax.set_xlabel('X [mm]')
    ax.set_ylabel('Y [mm]')
    ax.set_zlabel('Z (Thickness) [mm]')
    ax.set_title('3D Deformation of Artificial Skin Sample', fontsize=12, fontweight='bold')
...
if __name__ == "__main__":
    ...
    # Stretching along the X-axis (+15%), causing the skin to narrow in Y (-4%) and THIN in Z (-4%)
    u_3d = np.array([
        [0.00,  0.00,  0.000],  # 1
        [0.15, -0.02, -0.004],  # 2
        [0.15, -0.04, -0.004],  # 3
    ...
    print("==================================================")
    print(f"Longitudinal stretching  (eps_xx): {eps_3d[0,0]:.4f}")
    print(f"Transverse compression   (eps_yy): {eps_3d[1,1]:.4f}")
    print(f"Z-axis thinning          (eps_zz): {eps_3d[2,2]:.4f}  <-- THIS IS THE THICKNESS EFFECT!")
    print("==================================================")
